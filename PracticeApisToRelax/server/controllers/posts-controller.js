...
module.exports = {
  createImgPost: async (req, res) => {
    ...
    try {
      ...
      if (!subcruddit) {
        return res
          .status(400)
          .send({ message: "you must post to a valid subcruddit" });
      }
    ...
    try {
      ...
      if (user.length < 1) {
        return res
          .status(400)
          .json({
            message: "Create post from valid user account",
            user: false,
          });
      }
      const subcruddit = await Subcruddits.findOne({
      ...
      if (!subcruddit) {
        return res
          .status(400)
          .json({ message: "Create post to valid subcruddit", user: false });
      }
      post.SubcrudditId = subcruddit.id;
      post.points = 1;
  ...
  findAllActivePostsSubcruddit: async (req, res) => {
    ...
    if (!sub) {
      return res
        .status(400)
        .send({ message: "that subcruddit does not exist" });
    }
    if (order === "new") {
      activePosts = await Posts.findAll({
        where: {
  ...
  editComment: async (req, res) => {
    try {
      ...
      if (!validator.isLength(comment.content, { min: 2, max: 5000 })) {
        res
          .status(400)
          .json({ message: "Comments must be between 2 and 5000 characters" });
      }
      ...
      if (post.UserId !== userId) {
        res
          ...
          .json({
            message:
              "You are not authorized to edit a comment that isn't yours.",
          });
      }
      ...
      post
        ...
        .then(() => {
          res
            .status(200)
            .send({ message: "Successfully edited comment", comment: post });
        })
        .catch((error) => {
          res
            .status(500)
    ...
    try {
      ...
      thePost
        ...
        .then(() => {
          return res
            .status(200)
            .send({ id: thePost.id, isActive: thePost.isActive });
        })
        .catch((error) => {
          return res
            .status(500)
            .send({ message: "Could not delete post", error: error });
        });
    } catch (error) {
      console.log(error);
      res.status(500).json({ message: "Internal Server Error" });
    ...
    try {
      ...
      thePost
        ...
        .then(() => {
          return res
            .status(200)
            .send({ id: thePost.id, isLocked: thePost.isLocked });
        })
        .catch((error) => {
          return res
            .status(500)
            .send({ message: "Could not lock post", error: error });
        });
    } catch (error) {
      console.log(error);
      res.status(500).json({ message: "Internal Server Error" });
    ...
    try {
      ...
      if (req.role === "user") {
        ...
        if (!mod) {
          return res
            .status(401)
            .send({
              message: "You do not have permission to sticky this post",
            });
        }
      }
      ...
      thePost
        ...
        .then(() => {
          return res
            .status(200)
            .send({ id: thePost.id, isStickied: thePost.isStickied });
        })
        .catch((error) => {
          return res
            .status(500)
            .send({ message: "Could not change post sticky", error: error });
        });
    } catch (error) {
      console.log(error);
      res.status(500).json({ message: "Internal Server Error" });
...
function validateImgPost(post, req, res) {
  ...
  if (
    ...
    res.status(400).send({
      message:
        "You must send user id, a post title, a post type, a subcruddit name, and post content .",
    });
    return false;
  }
...
function validateTextPost(post, req, res) {
  if (!post.UserId || !post.title || !post.postType || !post.content) {
    res.status(400).send({
      message:
        "You must send user id, a post title, a post type, and post content .",
    });
    return false;
  }
  if (post.parentId || post.postId) {
...
function validateComment(comment, postId, req, res) {
  if (!postId || !comment.UserId || !comment.content) {
    res.status(400).send({
      message:
        "You must send user id, subcruddit id, a post type, and post content .",
    });
    return false;
  }
  if (!validator.isLength(comment.content, { min: 2, max: 5000 })) {
